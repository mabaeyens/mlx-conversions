...
- [x] ~~Set up the weekly candidate-scan routine~~ — **removed 2026-09-05**. The routine (cloud trigger `trig_01X1Z7iv5vwZVLwyG423YY82`, Saturdays 07:00 UTC) could never actually reach the HF API from its execution environment (`huggingface.co` is blocked by that environment's network egress policy — confirmed on its first real run, 2026-09-05), so it was pure overhead. The trigger itself needs to be deleted from the claude.ai/code scheduled-tasks UI (no session-level tool can remove a durable cloud trigger); `scripts/scan_candidates.py` is left in place for manual, local runs. See specs/conversion-queue-automation.md for status.
